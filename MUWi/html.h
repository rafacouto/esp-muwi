...
<head>
  ...
  <script type='text/javascript' >
    ...
    var mouseClickMillis;
    function mouseDown(control) {
      mouseClickMillis = (new Date()).getTime();
    }
    function mouseUp(control) {
      var now = (new Date()).getTime();
      var alternative = (now - mouseClickMillis > 1000);
      var cmd = alternative ? (control.id + control.id) : control.id;
      xhr.open('GET', 'accion?comando=' + cmd);
      xhr.send();
    }
  </script>
</head>
